...
> **Note:** `C — Label AbuseIPDB` is currently **disabled** — AbuseIPDB output passes through to Merge directly. Re-enable if you need explicit labelling for that engine.
...
| `C — Label AbuseIPDB` | Code | Tag AbuseIPDB result *(disabled)* |
...
- **AbuseIPDB label node is disabled** — re-enable `C — Label AbuseIPDB` if needed
